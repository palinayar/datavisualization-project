...
function initTimeGrid() {
  // Load the CSV data to calculate average views
  d3.csv("../data/sentiments_CAvideos.csv").then(data => {
    ...
    const monthViews = d3.rollup(data, v => d3.mean(v, d => d.views), d => d.publish_time.getMonth());
...
function createDayGrid(month) {
  d3.csv("../data/sentiments_CAvideos.csv").then(data => {
    ...
    const dayViews = d3.rollup(data.filter(d => d.publish_time.getMonth() === monthIndex),
      v => d3.mean(v, d => d.views), d => d.publish_time.getDate());
    ...
    svg.selectAll("text.day")
      ...
      .attr("y", (d, i) => Math.floor(i / cols) * (rectSize + padding) + rectSize / 2 + 150 + offset)
      .attr("dy", ".35em")
      .attr("text-anchor", "middle")
      .attr("fill", "white")
      .text(d => d)
      ...
      .duration(1000)
      .attr("x", (d, i) => (i % cols) * (rectSize + padding) + rectSize / 2)
      .attr("y", (d, i) => Math.floor(i / cols) * (rectSize + padding) + rectSize / 2 + offset);
